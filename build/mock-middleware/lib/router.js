...
Router.prototype.loadRoutes = function(routeFile) {
  const mapping = require(routeFile)
...
Router.prototype.search = function(url, method) {
  ...
  if (match) {
    let type = 'view'
    const urlExp = /(http|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&amp:/~\+#]*[\w\-\@?^=%&amp/~\+#])?/
